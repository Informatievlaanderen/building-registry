...
public sealed class GivenBuildingsToMergeExists : BuildingRegistryTest
{
    ...
    {
        // create multiple buildings w units
    }
        ...
        var result = Building.MergeBuildings(
            ...
            new List<Building>
            {
                Building.RealizeAndMeasureUnplannedBuilding(
                    new BuildingFactory(NoSnapshotStrategy.Instance),
                    new BuildingPersistentLocalId(command.NewBuildingPersistentLocalId + 1),
                    Fixture.Create<ExtendedWkbGeometry>(),
                    Fixture.Create<BuildingGrbData>()),
                Building.RealizeAndMeasureUnplannedBuilding(
                    new BuildingFactory(NoSnapshotStrategy.Instance),
                    new BuildingPersistentLocalId(command.NewBuildingPersistentLocalId + 2),
                    Fixture.Create<ExtendedWkbGeometry>(),
                    Fixture.Create<BuildingGrbData>())
            });
