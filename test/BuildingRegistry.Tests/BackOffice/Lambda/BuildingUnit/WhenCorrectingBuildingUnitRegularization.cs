namespace BuildingRegistry.Tests.BackOffice.Lambda.BuildingUnit
{
    ...
    public class WhenCorrectingBuildingUnitRegularization : BackOfficeLambdaTest
    {
        private readonly IdempotencyContext _idempotencyContext;

        public WhenCorrectingBuildingUnitRegularization(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        [Fact]
        public async Task ThenBuildingUnitIsRegularizationIsCorrected()
        {
            // Arrange
            ...
            PlanBuildingUnit(buildingPersistentLocalId, buildingUnitPersistentLocalId, hasDeviation: false);

            var eTagResponse = new ETagResponse(string.Empty, Fixture.Create<string>());
            var handler = new CorrectBuildingUnitRegularizationLambdaHandler(
            ...
            //Act
            await handler.Handle(new CorrectBuildingUnitRegularizationLambdaRequest(
                buildingPersistentLocalId.ToString(),
                new CorrectBuildingUnitRegularizationSqsRequest
                {
                    TicketId = Guid.NewGuid(),
                    Request = new CorrectBuildingUnitRegularizationRequest { BuildingUnitPersistentLocalId = buildingUnitPersistentLocalId },
        ...
        public async Task WhenIdempotencyException_ThenTicketingCompleteIsExpected()
        {
            ...
            ticketing.Verify(x =>
                x.Complete(
                    It.IsAny<Guid>(),
                    new TicketResult(new ETagResponse(string.Format(ConfigDetailUrl, buildingUnitPersistentLocalId), building.LastEventHash)),
                    CancellationToken.None));
        }
