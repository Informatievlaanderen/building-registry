namespace BuildingRegistry.Tests.BackOffice.Sqs.BuildingUnit
{
    ...
    public sealed class GivenChangeBuildingUnitFunctionRequest : BuildingRegistryTest
    {
        private readonly FakeBackOfficeContext _backOfficeContext;

        public GivenChangeBuildingUnitFunctionRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sut = new ChangeBuildingUnitFunctionSqsHandler(
                sqsQueue.Object,
                ticketingMock.Object,
                ticketingUrl,
                _backOfficeContext);

            var sqsRequest = new ChangeBuildingUnitFunctionSqsRequest
            {
                BuildingUnitPersistentLocalId = Fixture.Create<BuildingUnitPersistentLocalId>(),
                Request = new ChangeBuildingUnitFunctionBackOfficeRequest
                {
        ...
        public void WithNoBuildingFoundByBuildingUnitPersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                new ChangeBuildingUnitFunctionSqsRequest
                {
                    ...
                    Request = new ChangeBuildingUnitFunctionBackOfficeRequest
                    {
                        Functie = GebouweenheidFunctie.DancingRestaurantCafe
                    }
                }, CancellationToken.None);
