namespace BuildingRegistry.Projector.Snapshots
{
    ...
    public class SnapshotsController : ApiController
    {
        ...
        private const string SnapshotVerificationsTableName = "[BuildingRegistry].[SnapshotVerificationStates]";
        private const string SnaphotsTableName = "[BuildingRegistry].[Snapshots]";
            ...
            var differenceInDaysOfLastVerification =
                await sqlConnection.QuerySingleAsync<int>(
                    $"SELECT DATEDIFF(DAY," +
                        ...
                        $"(SELECT MAX(Created) FROM [BuildingRegistry].[Snapshots])) As DaysDiff",
                    cancellationToken);
