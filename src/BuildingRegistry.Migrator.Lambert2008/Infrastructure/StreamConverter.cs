namespace BuildingRegistry.Migrator.Lambert2008.Infrastructure
{
    ...
    {
        private const string StreamIdPrefix = "building-";
        ...
        public async Task ProcessAsync(CancellationToken ct)
        {
            ...
            var totalStreams = await _sqlStreamsTable.CountBuildingStreams(ct);
            var alreadyProcessed = await _processedStreamsTable.GetProcessedCount(ct);

            _logger.LogInformation(
                "Starting {Mode} over {TotalStreams} building streams, {AlreadyProcessed} already done, "
            ...
            var convertedBuildings = 0;
            var pageNumber = 0;
            var stoppedEarly = false;

            var queryStarted = Stopwatch.GetTimestamp();
            var pageOfStreams = (await _sqlStreamsTable.ReadNextBuildingStreamPage(lastCursorPosition)).ToList();
            ...
            while (pageOfStreams.Count != 0)
            {
                ...
                convertedBuildings += pageResults.Count(x => x.WasConverted);
                ...
                pageOfStreams = (await _sqlStreamsTable.ReadNextBuildingStreamPage(lastCursorPosition)).ToList();
            ...
            CancellationToken ct)
        {
            ...
            await Parallel.ForEachAsync(streams, parallelOptions, async (stream, innerCt) =>
            {
                ...
                var buildingPersistentLocalId = ParseBuildingPersistentLocalId(streamId);

                try
                {
                    var result = await ConvertStream(buildingPersistentLocalId, innerCt);
                    ...
                    await _processedStreamsTable.Add(internalId, buildingPersistentLocalId, result);
        ...
        private async Task<StreamConversionResult> ConvertStream(int buildingPersistentLocalId, CancellationToken ct)
        {
            await using var streamLifetimeScope = _lifetimeScope.BeginLifetimeScope();

            var loadStarted = Stopwatch.GetTimestamp();

            var buildings = streamLifetimeScope.Resolve<IBuildings>();
            var building = await buildings.GetAsync(
                new BuildingStreamId(new BuildingPersistentLocalId(buildingPersistentLocalId)),
                ct);

            var buildingUnits = building.BuildingUnits;
            var geometryToConvert = !IsLambert2008(building.BuildingGeometry.Geometry);
            var buildingUnitsToConvert = buildingUnits
                .Count(buildingUnit => !IsLambert2008(buildingUnit.BuildingUnitPosition.Geometry));

            var loadDuration = Stopwatch.GetElapsedTime(loadStarted);

            if ((!geometryToConvert && buildingUnitsToConvert == 0) || _dryRun)
            {
                return new StreamConversionResult(
                    buildingUnits.Count,
                    buildingUnitsToConvert,
                    geometryToConvert,
                    loadDuration,
                    TimeSpan.Zero);
            }
        ...
        private static Provenance CreateProvenance()
            => new Provenance(
                ...
                new Reason("Omzetting van de gebouwgeometrie naar Lambert 2008 (EPSG 3812)."),
        ...
        private void LogStream(string streamId, StreamConversionResult result)
        {
            const string message =
                "Stream '{StreamId}': {BuildingUnitCount} building units, geometry to convert {GeometryToConvert}, "
                + "{BuildingUnitsToConvert} positions to convert, took "
            ...
            _logger.Log(
                ...
                message,
                streamId,
                result.BuildingUnitCount,
                result.GeometryToConvert,
                result.BuildingUnitsToConvert,
            ...
            TimeSpan elapsed)
        {
            ...
            _logger.LogInformation(
                "Page {PageNumber}: {PageStreamCount} streams ({ProcessedInPage} processed, {BuildingsInPage} to convert) "
                + "in {PageSeconds:F1}s at {StreamsPerSecond:F1} streams/s, paging query {QueryMilliseconds}ms. "
                + "Progress {DoneStreams}/{TotalStreams} ({ProgressPercentage:F1}%), {ConvertedBuildings} buildings "
                + "this run, elapsed {Elapsed:g}, ETA {EstimatedTimeLeft}.",
                pageNumber,
                pageStreamCount,
                pageResults.Count,
                pageResults.Count(x => x.WasConverted),
            ...
            bool stoppedEarly)
        {
            ...
            var buildingsPerSecond = elapsed.TotalSeconds > 0 ? convertedBuildings / elapsed.TotalSeconds : 0;

            _logger.LogInformation(
                "{Outcome} after {Elapsed:g}: {ProcessedThisRun} streams processed this run, "
                + "{ConvertedBuildings} buildings {Verb}, at {StreamsPerSecond:F1} streams/s and "
                + "{BuildingsPerSecond:F0} buildings/s. "
                + "{DoneStreams}/{TotalStreams} buildings done overall, next run resumes at internal id {Cursor}.",
        ...
        private static bool IsLambert2008(ExtendedWkbGeometry geometry)
            => geometry.ToByteArray().TryReadSrid(out var srid) && srid == SystemReferenceId.SridLambert2008;

        private static int ParseBuildingPersistentLocalId(string streamId)
        {
            if (!streamId.StartsWith(StreamIdPrefix, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException($"Stream '{streamId}' is not a building stream.");
            }
