namespace BuildingRegistry.Migrator.Lambert2008.Infrastructure
{
    ...
    /// <summary>
    /// Bookkeeping of the building streams that have been converted, so a restart resumes instead of
...
CREATE TABLE {Table}(
    ...
    [BuildingPersistentLocalId] [int] NOT NULL,
    [BuildingUnitCount] [int] NOT NULL,
    [BuildingUnitsToConvert] [int] NOT NULL,
    [GeometryToConvert] [bit] NOT NULL,
    ...
    CONSTRAINT [PK_Lambert2008ProcessedStreams] PRIMARY KEY CLUSTERED ([Id] ASC, [IsDryRun] ASC)
        ...
        /// afterwards — cost against unit count, the slowest streams, percentiles — instead of having to be
            ...
            StreamConversionResult result)
        {
            try
            {
                ...
                await connection.ExecuteAsync(
                    $@"INSERT INTO {Table} (Id, IsDryRun, BuildingPersistentLocalId, BuildingUnitCount, BuildingUnitsToConvert, GeometryToConvert, LoadMilliseconds, DispatchMilliseconds, IsPageCompleted, ProcessedAt)
                       VALUES (@Id, @IsDryRun, @BuildingPersistentLocalId, @BuildingUnitCount, @BuildingUnitsToConvert, @GeometryToConvert, @LoadMilliseconds, @DispatchMilliseconds, 0, SYSDATETIMEOFFSET())",
                    new
                    {
                        Id = internalId,
                        IsDryRun = _dryRun,
